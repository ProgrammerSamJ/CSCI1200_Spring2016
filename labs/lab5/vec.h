...
public:   
  ...
  void shrink(size_type n, const T& fill_in_value = T());
...
template <class T> void  Vec<T>::print() {
  ...
  cout << "Allocated:" << m_alloc <<endl;
  cout << "Size:" << m_size<<endl;
}
//shrink function
template <class T> void Vec<T>::shrink(size_type n, const T& fill_in_value) { 
  if (n < m_alloc) {
    m_alloc = n;
    T* new_data = new T[m_alloc];
    for (size_type i=0; i<m_size; ++i)
...
//Removing numbers that appear more than once
template <class T> typename Vec<T>::size_type remove_matching_elements(Vec<T>& v,T n) {
  typename Vec<T>::size_type counter = 0;
