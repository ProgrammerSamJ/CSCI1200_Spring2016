...
bool find_roots(int a, int b, int c, float &root_pos, float &root_neg) {
  ...
  // if the radical is negative, the roots are imaginary (not used in checkpoint 2,we dont care if they are imaginary)
  if (radical < 0) {
...
int main() {
  ...
  int b_array[5];
  int c_array[5];
  // Getting the integers for each polynomial
  for (int n=0;n<5;n++) {
    std::cout << "Enter a b coefficient" << std::endl;
    int my_b;
    std::cin >> my_b;
    b_array[n] = my_b;
  }
  for (int n=0;n<5;n++) {
    std::cout << "Enter a c coefficient" << std::endl;
    int my_c;
    std::cin >> my_c;
    c_array[n] = my_c;
  }
  for (int n=0;n<5;n++) {
    ...
    my_b = b_array[n];
    my_c = c_array[n];
    std::cout << my_b << " and " << my_c << std::endl;

    float root_1, root_2;
    bool success = find_roots(1,my_b,my_c, root_1,root_2);
    if (success == true) {
      smaller_root[n] = std::min(root_1,root_2);
      std::cout << "******"<< smaller_root[n] << "******" <<std::endl;
      }
    }
  return 0;
}
