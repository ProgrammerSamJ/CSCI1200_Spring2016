...
public:
  ...
  Rectangle(const Point2D& lower_left, const Point2D& upper_right); //2 sets of point objects are the inputs to give 
  // give the borders of the rectangle
  //  Access the corners of the rectangle and the points stored in the rectangle.
  const Point2D& upper_right_corner() const { return m_upper_right; } //returns the upper right corner
  const Point2D& lower_left_corner() const { return m_lower_left; }  //returns the upper left corner
  const std::vector<Point2D>& points_contained() const { return m_points_contained; } //list of point objects that are
