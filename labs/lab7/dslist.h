...
public:
  // default constructor, copy constructor, assignment operator, & destructor
  dslist() : head_(NULL), tail_(NULL), end_(new Node<T>),size_(0) {}
...
void dslist<T>::destroy_list() {
  iterator it;
  Node<T>* current = head_;       //start at the head
  Node<T>* next;
  while (current != NULL){        //while we have not reached the tail(end of DL)
    next = current -> next_;      //the next element will be the current element's next
    delete current;               //delete the current element
    current = next;               //the current element is now the next element,this is relinking the list
  }
  head_ = NULL;                   //when we are done, point the head to nothing
  size_ = 0;
}
