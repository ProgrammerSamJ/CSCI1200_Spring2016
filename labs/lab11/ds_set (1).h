...
public:
  ...
  tree_iterator<T> & operator--() {
    ...
    else if (ptr_->left != NULL) { // find the righttmost child of the right node
      ptr_ = ptr_->left;
      while (ptr_->right != NULL) { ptr_ = ptr_->right; }
    } else { // go upwards along right branches...  stop after the first right
...
public:
  ...
  void print_as_sideways_tree(std::ostream& ostr) const {
   ...
   void accumulate(T &x){
    if (root_ != NULL){
      x += root_->value;
      TreeNode<T>* l = root_->left;
      TreeNode<T>* r = root_->right;

      while(r != NULL){
        x += r->value;
        if(r->left != NULL){
          x+=r->left->value;
          if(r->left->left != NULL){ x += r->left->left->value; }
          if(r->left->right != NULL){ x += r->left->right->value; }     
        }
        r= r->right;
      ...
      while(l != NULL){
        ...
        if(l->right != NULL){
          x+=l->right->value;
          if(l->right->left != NULL){ x += l->right->left->value; }
          if(l->right->right != NULL){ x += l->right->right->value; }    
        }
        l = l->left;
      }
    }
