...
void find_paths(vector<vector<bool> > empty_grid,int x,int y,int &b){
  if ((x == 0 && y == 0)){
    b++;
    return;
  }
  if (empty_grid[x][y] == true){
    return;
  }
  if ((x > 0) && (empty_grid[x-1][y] == false)){ 
    find_paths(empty_grid,x-1,y,b);
  }
  if ((y > 0) && (empty_grid[x][y-1] == false)){
    find_paths(empty_grid,x,y-1,b);
  }
}
void find_one_path(vector<vector<bool> > empty_grid,int x,int y,int &b,vector<Point> &Path){
  if ((x == 0 && y == 0)){
    b++;
    return;
  }
  ...
  if ((x > 0) && (empty_grid[x-1][y] == false)){
    Point tempPoint(x-1,y); 
    find_one_path(empty_grid,x-1,y,b,Path);
    if (b==1){
  ...
  if ((y > 0) && (empty_grid[x][y-1] == false)){
    Point tempPoint(x,y-1);
    find_one_path(empty_grid,x,y-1,b,Path);
    if (b==1){
       Path.push_back(tempPoint);
      return;
    }
...
int main(int argc, char* argv[]) {
  ...
  find_paths(blocked_grid,start_x,start_y,b);
 
  cout << "paths:" << b <<endl; 
  b =0;
  find_one_path(blocked_grid,start_x,start_y,b,Path);
  cout << Path.size() <<endl;
  for(int i = 0; i < Path.size();i++){
    cout<< "Point:" << Path[i].x << Path[i].y <<endl;
  }
