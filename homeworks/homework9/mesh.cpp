...
// once collapse is working, start playing with the priority queue
bool Mesh::Collapse() {
  Edge *e = FindEdge();
  if (e == NULL) { 
  ...
  Vertex *a = e->getV1();
  Vertex *b = e->getV2();
  // temporary variable to save the original a
  Vertex *tempA = a;
  ...
  a->Average(b);  // a now represents the midpoint between the two edges

  //
  // ASSIGNMENT: Finish the implementation of this function
  //
  //remove edge where b is an a is not
  //set the one with b to the middle

  // temporary triangle set, will contain the triangles after the edge collapse
  // we will check each triangle (t) to see if it has both a and b as vertices or just one of a and b as vertices
  triangles_set copy_tri = triangles;
  for (triangles_set::iterator itr = copy_tri.begin(); itr != copy_tri.end(); itr++) {
    Triangle *t = *itr;
    // remove the triangles that have ab as an edge
    if (t->HasVertex(tempA) && t->HasVertex(b)) {
      // save the unique vertex and use it later to add the new edge
      Vertex* third;
      for(int i = 0; i < 3;i++){
        // when we find the unique vertex, save it and remove  triangele and its edges,then add the adjusted edge
        if ( (t->getVertex(i) != tempA) && (t->getVertex(i) != b) ) {
          third = t->getVertex(i);
          // remove the triangle's edges from the set
          RemoveEdge(tempA,b);
          RemoveEdge(tempA,third);
          RemoveEdge(b,third);
          // remove the triangle from the set after all operations are performed
          RemoveTriangle(t);
          // adding the adjusted edge to 
          AddEdge(a,third);
          break;
        }
      }
    }
    // change every triangle that has a or b(not both) as a vertex but
    // if a vertex is a or b,a new triangle will be added,that has the triangle's old vertices and the 
    // vertex that was a or b will be replaced with the average
    else if (t->HasVertex(b)) {
      // remove all edges tounching b
      // save the triangles vertices
      std::vector<Vertex*> vertices;
      // if b is the vertex, remove all the edges, save all the vertices, but replace b with a
      RemoveEdge(t->getVertex(0),t->getVertex(1));
      RemoveEdge(t->getVertex(1),t->getVertex(2));
      RemoveEdge(t->getVertex(2),t->getVertex(0));
      for(int i = 0; i < 3;i++){
        if (t->getVertex(i) == b){ vertices.push_back(a);}
        else vertices.push_back(t->getVertex(i));
      }
      // remove the triangle from the set after all operations are performed
      RemoveTriangle(t);
  
      // add a new triangle, 2 adjusted edges, and the last original edge
      AddTriangle(vertices[0],vertices[1],vertices[2]);
    }
  }
   // set to save the edges we will recalculate
   std::set<Edge*> collected_edges;
   // collect the edges with a as a vertex
   CollectEdgesWithVertex(a,collected_edges);
    
  for (triangles_set::iterator itr = triangles.begin(); itr != triangles.end(); itr++) {
    Triangle *t = *itr;
    CollectEdgesWithVertex( t->getVertex(0),collected_edges);
    CollectEdgesWithVertex( t->getVertex(1),collected_edges);
    CollectEdgesWithVertex( t->getVertex(2),collected_edges);
  }
