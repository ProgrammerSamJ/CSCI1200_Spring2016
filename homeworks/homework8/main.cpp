...
void student_tests() {
  ...
  simple_points.push_back(std::make_pair(Point<int>(10,5), 'B'));
  simple_points.push_back(std::make_pair(Point<int>(30,4), 'C'));
  simple_points.push_back(std::make_pair(Point<int>(30,4), 'D'));
  simple_points.push_back(std::make_pair(Point<int>(10,3), 'E'));
  simple_points.push_back(std::make_pair(Point<int>(13,5), 'F'));

  std::vector< std::pair<Point<double>,std::string> > double_points;
  double_points.push_back(std::make_pair(Point<double>(20.01,10.32), "A"));
  double_points.push_back(std::make_pair(Point<double>(10.12,5.83), "B"));
  double_points.push_back(std::make_pair(Point<double>(30.52,4.38), "C"));

   std::cout<< "Testing a tree of integers with 3 levels" <<std::endl;
  //INSERT TESTS
  ...
  simple.plot(40,20);
  //insert a duplicate
  std::cout<< "Trying to add a duplicate" <<std::endl;
  std::pair<QuadTree<int,char>::iterator,bool> temp = simple.insert(simple_points[3].first,simple_points[3].second);
  assert(temp.second == false);
  simple.plot(40,20);

  std::cout<<"\nIterator Tests:" <<std::endl;
  QuadTree<int,char>::iterator temp1 = simple.begin();
  std::cout<< "going down one level(depth)" <<std::endl;
  std::cout <<"first: "<< temp1.getLabel() <<std::endl;
  temp1++;
  std::cout << "next: "<< temp1.getLabel() <<std::endl;
  QuadTree<int,char>::bf_iterator temp2 = simple.bf_begin();
  std::cout<< "going down one level(breadth)" <<std::endl;
  std::cout <<"first: "<< temp2.getLabel() <<std::endl;
  temp2++;
  std::cout << "next: "<< temp2.getLabel() <<std::endl;

  //QUAD TREE OF DOUBLES
  std::cout<< "Testing a tree of doubles with 3 levels and strings as labels" <<std::endl;
  std::cout<<"adding elements:";
  QuadTree<double,std::string> doubles;
  for (unsigned int i = 0; i < double_points.size(); i++) {
    doubles.insert(double_points[i].first,double_points[i].second);
    assert(doubles.size() == i+1);
  }
  std::cout<<"it worked!" <<std::endl;

  //COPY CONSTRUCTOR
  std::cout<<"\nCopy Test:"<<std::endl;
  QuadTree<int,char> copied_tree(simple);
  assert(copied_tree.size() == simple.size());
  std::cout<<"original:"<<std::endl;
  simple.plot(40,20);
  std::cout<<"\ncopy:"<<std::endl;
  copied_tree.plot(40,20);
