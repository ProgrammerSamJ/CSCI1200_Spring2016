...
public:
  ...
  iterator find2(const T& key_value) { return find(key_value, root_); }
...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    TreeNode<T>* p = old_root;        //head of the new tree
    if(p == NULL)                     //if we reach null,we are at the end of the old tree
      return p;     
    if(p->left != NULL){              //while the old tree still has a left
      p->left = old_root->left;       //new tree's left = old tree's left
      copy_tree(old_root->left);      //make the old tree's left the new start,then repeat the process
    }
    if(p->right != NULL){
      p->right = old_root->right;
      copy_tree(old_root->right);
    }
  }
...
destroy_tree(root_);
  ...
  iterator find2(const T& key_value, TreeNode<T>* p){
    while(p != NULL){
      if (p==NULL)
